...
export async function getTransactions(walletAddress: string, limit: number = 100): Promise<SolanaTransaction[]> {
  try {
    ...
    // Check if this is the sample address, and if so, return predefined sample data
    if (walletAddress === SAMPLE_ADDRESS) {
      console.log('Using sample data for the demo address');
    ...
    // Only return sample data for the sample address
    if (walletAddress === SAMPLE_ADDRESS) {
      return getSampleTransactionsForDemo();
    }
